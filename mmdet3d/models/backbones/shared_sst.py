...
class SharedSST(nn.Module):
    '''
    Shared Single-stride Sparse Transformer. 
    Main args:
    ...
    def forward(self, sst_info):
        '''
        '''
        num_shifts = 2 
        assert sst_info['sst_coors'].dtype == torch.int64, 'data type of coors should be torch.int64!'

        device = sst_info['sst_coors'].device
        batch_size = sst_info['sst_coors'][:, 0].max().item() + 1
        sst_feat = sst_info['sst_feats']
        ind_dict_list = [sst_info[f'flat2win_inds_shift{i}'] for i in range(num_shifts)]
        padding_mask_list = [sst_info[f'key_mask_shift{i}'] for i in range(num_shifts)]
        pos_embed_list = [sst_info[f'pos_dict_shift{i}'] for i in range(num_shifts)]
        ...
        if not self.masked:
            output = self.recover_bev(output, sst_info['voxel_coors'], batch_size)
    ...
    def recover_bev(self, sst_feat, voxel_coors, batch_size):
        '''
        Args:
            sst_feat: shape=[N, C]
            voxel_coors: [N, 4]
        ...
        for batch_itt in range(batch_size):
            ...
            canvas = torch.zeros(
                feat_dim,
                nx * ny,
                dtype=sst_feat.dtype,
                device=sst_feat.device)

            # Only include non-empty pillars
            batch_mask = voxel_coors[:, 0] == batch_itt

            this_coors = voxel_coors[batch_mask, :]
            indices = this_coors[:, 2] * nx + this_coors[:, 3]
            indices = indices.type(torch.long)
            voxels = sst_feat[batch_mask, :] #[n, c]
            voxels = voxels.t() #[c, n]
